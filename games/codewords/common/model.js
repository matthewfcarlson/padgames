...
//https://github.com/proin/word2vec.js/blob/master/libs/bin/binary2txt.c
...
module.exports = function loadModel( data, callback ) {
	var model = {};
	var vocab = [];
	var words;
	var size;
    ...
    model.GetWordList = function(){
        return vocab.map(x => x.word);
    };

	model.getVector = function ( word ) {
		for ( var i = 0; i < words; i++ ) {
			if ( vocab[i].word.toLowerCase() == word.toLowerCase() ) {
				return vocab[i];
			}
		}
		return null;
	...
	function init( newData ) {
        //TODO fix this to be more resilent
        words = newData.length;
        model.words = words;
        
        for (var i=0;i<words;i++){
            if (i==0) model.size = newData[i].vectors.length;
            var o = new WordVec(newData[i].name, newData[i].vectors);
            vocab.push(o);
        }
        size = model.size;
